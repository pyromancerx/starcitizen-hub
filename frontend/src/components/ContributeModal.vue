<template>
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
    <div class="bg-sc-dark p-6 rounded-lg shadow-lg w-full max-w-md border border-sc-grey/20">
      <h3 class="text-xl font-bold text-white mb-4">Contribute to Goal</h3>
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          <label for="contributionAmount" class="block text-sc-grey text-sm font-bold mb-2">Amount</label>
          <input type="number" id="contributionAmount" v-model.number="contribution.amount"
        ...
        <div>
          <label for="notes" class="block text-sc-grey text-sm font-bold mb-2">Notes (Optional)</label>
          <textarea id="notes" v-model="contribution.notes" rows="3"
...
const props = defineProps({
  show: Boolean,
  goalId: Number // The ID of the goal being contributed to
});
...
const handleSubmit = () => {
  if (contribution.value.amount <= 0) {
    alert('Contribution amount must be greater than zero.');
    return;
  }
  emit('contribute', { ...contribution.value });
