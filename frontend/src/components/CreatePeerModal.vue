<template>
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
    <div class="bg-sc-dark p-6 rounded-lg shadow-lg w-full max-w-md border border-sc-grey/20">
      <h3 class="text-xl font-bold text-white mb-4">Create Peer Link</h3>
...
const emit = defineEmits(['close', 'peer-created']); // Changed event name

const peerData = ref({ // Changed ref name
  name: '',
  base_url: '',
  api_key: ''
});
...
const handleSubmit = () => {
  emit('peer-created', { ...peerData.value }); // Changed event name
