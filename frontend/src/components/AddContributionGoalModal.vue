<template>
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
    <div class="bg-sc-dark p-6 rounded-lg shadow-lg w-full max-w-md border border-sc-grey/20">
      <h3 class="text-xl font-bold text-white mb-4">Add New Contribution Goal</h3>
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          <label for="resourceType" class="block text-sc-grey text-sm font-bold mb-2">Resource Type</label>
          <input type="text" id="resourceType" v-model="newGoal.resource_type"
        ...
        <div>
          <label for="targetAmount" class="block text-sc-grey text-sm font-bold mb-2">Target Amount</label>
          <input type="number" id="targetAmount" v-model.number="newGoal.target_amount"
        ...
        <div>
          <label for="unit" class="block text-sc-grey text-sm font-bold mb-2">Unit (e.g., UEC, units, hours)</label>
          <input type="text" id="unit" v-model="newGoal.unit"
                 class="shadow appearance-none border border-sc-grey/30 rounded w-full py-2 px-3 bg-sc-panel text-white leading-tight focus:outline-none focus:shadow-outline focus:border-sc-blue" required />
        </div>
...
const emit = defineEmits(['close', 'add-goal']);
