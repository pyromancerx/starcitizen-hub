<template>
  <div v-if="show" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
    <div class="bg-sc-dark p-6 rounded-lg shadow-lg w-full max-w-md border border-sc-grey/20">
      <h3 class="text-xl font-bold text-white mb-4">Add New Project Phase</h3>
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          <label for="phaseName" class="block text-sc-grey text-sm font-bold mb-2">Phase Name</label>
          <input type="text" id="phaseName" v-model="newPhase.name"
        ...
        <div>
          ...
          <textarea id="description" v-model="newPhase.description" rows="3"
        ...
        <div>
          <label for="sortOrder" class="block text-sc-grey text-sm font-bold mb-2">Sort Order</label>
          <input type="number" id="sortOrder" v-model.number="newPhase.sort_order"
