        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 group">
                <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-sc-grey/40 italic">Primary designation for your Hub instance.</p>
                </div>
                <div className="md:col-span-2 flex items-center space-x-3">
                    <input 
                        defaultValue={settings?.find((s: any) => s.key === 'org_name')?.value}
                        onBlur={(e) => {
                            const current = settings?.find((s: any) => s.key === 'org_name')?.value;
                            if (e.target.value !== current) {
                                updateSettingMutation.mutate({ key: 'org_name', value: e.target.value });
                            }
                        }}
